...
const Exercise = props => (
  <tr>
    ...
    <td>{props.exercise.departement}</td>
    ...
    var type = this.state.recherche_type;
      ...
      .catch((error) => {
        ...
        if(type =="user"){
          ...
          this.setState({
            exercises :this.state.exercises.filter(ex=>ex.departement.toLowerCase().startsWith(val.toLowerCase()))
          })
          console.log("je");
        }
  ...
  render() {
    return (
      <div>
        ...
        <center>
        <label><i>Recherche</i></label>&nbsp;
           <input type='text' onChange={this.changer} style={{height:"25px"}}/>
           <select onChange={(e) => this.setState({recherche_type:e.target.value})} value={this.state.recherche_type}>
